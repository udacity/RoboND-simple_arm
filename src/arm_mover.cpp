...
std::vector<float> clamp_at_boundaries(float requested_j1, float requested_j2)
{
    ...
    // Define clamped joint angles and assign them to requested ones
    ...
    // Assign a new node handle since I have no access to the main one
    ros::NodeHandle n2;
    // Get node name
    ...
    // Check if each joint falls in the safe zone, otherwise clamp it
    ...
    std::vector<float> clamped_data;
    clamped_data.push_back(clamped_j1);
    clamped_data.push_back(clamped_j2);
    ...
    simple_arm::GoToPosition::Response& res)
{
    ...
    res.msg_feedback = "Joint angles set";
